...
def deactivate_order(order_id):
    order = models.ProxyOrder.objects.get(pk=order_id)
    ...
    models.Proxy.objects.filter(proxy_id__in=proxy_ids).delete()
    send_delay_message.delay(user_id=order.user.user_id, msg_name='Прокси удален')  


@app.task(ignore_result=True)
def run_auto_prolong_task(order_id):
    msg_name, user_id = models.ProxyOrder.prolong_order(order_id)
...
def prolong_orders_task(order_ids, period):
    for order_id in order_ids:
        msg_name, user_id = models.ProxyOrder.prolong_order(int(order_id), period)
        send_delay_message.delay(user_id=user_id, msg_name=msg_name)
...
def ckeck_orders_task(order_ids):
    ...
    for order_id in order_ids:
        user_id = models.ProxyOrder.check_order(int(order_id))
        send_delay_message.delay(user_id=user_id, msg_name=msg_name)
...
def change_order_task(order_ids):
    ...
    for order_id in order_ids:
        user_id = models.ProxyOrder.change_order(int(order_id))
        send_delay_message.delay(user_id=user_id, msg_name=msg_name)
...
def buy_proxy_task(user_id):
    ...
    msg_text = models.ProxyOrder.create_new_order(user_id=user_id, **kwargs)
